...
class BaseKernelSolver:
    ...
    def get_target_coords(self, targets):
        if self.num_labels == 1:  # Regression
            ...
            return torch.nn.functional.one_hot(targets.flatten(), self.num_labels).to(
                self.kernel_dtype
            )
    ...
    def loss(self, preds, targets):
        ...
        return (
            1 / targets_coords.shape[0] * ((preds - targets_coords) ** 2).sum().item()
        )
...
class LstsqKernelSolver(BaseKernelSolver):
    ...
    def fit(self, train_kernel, train_targets, train_logits=None):
        ...
        self.kernel_solution, self.residuals, self.rank, _ = torch.linalg.lstsq(
            kernel, train_targets_coords.t()
        )
    ...
    def predict(self, eval_kernel, eval_targets, eval_logits=None, **unused_kwargs):
        ...
        assert (
            eval_kernel.size(0) == self.num_labels
        ), "Number of labels in eval_kernel must match fit()"

        eval_preds = (
            torch.bmm(eval_kernel.transpose(1, 2), self.kernel_solution.unsqueeze(2))
            .squeeze(2)
            .transpose(0, 1)
        )  # shape [#dataset_outer, #classes]
...
class AsymmetricLstsqKernelSolver(LstsqKernelSolver):
    ...
    def fit(self, train_kernel, train_targets, train_logits=None):
        ...
        assert (
            self.num_labels == 1
        ), "SVMKernelSolver only works for regression tasks or binary_classification"
    ...
    def predict(
        ...
        **unused_kwargs
        ...
        omega = (
            torch.bmm(eval_kernel.transpose(1, 2), (alpha * self.Y).unsqueeze(2))
            .squeeze(2)
            .transpose(0, 1)
        )  # + alpha_bias
        nu = (
            torch.bmm(eval_kernel_flipped.transpose(1, 2), (beta * self.Y).unsqueeze(2))
            .squeeze(2)
            .transpose(0, 1)
        )  # + beta_bias

        eval_preds = (
            self.args.kernel_lambda * omega + (1 - self.args.kernel_lambda) * nu
        )
...
class SVRKernelSolver(BaseKernelSolver):
    ...
    def fit(self, train_kernel, train_targets, train_logits=None):
        ...
        for k in range(self.num_labels):
            ...
            svm.fit(
                kernel[k].cpu().numpy(), train_targets_coords[:, k].cpu().t().numpy()
            )
    ...
    def predict(self, eval_kernel, eval_targets, eval_logits=None, **unused_kwargs):
        ...
        for k in range(self.num_labels):
            ...
            eval_preds.append(
                torch.tensor(
                    predict_k, dtype=self.kernel_dtype, device=eval_kernel.device
                )
            )
...
class SVCKernelSolver(BaseKernelSolver):
    ...
    def fit(self, train_kernel, train_targets, train_logits=None):
        ...
        assert (
            self.num_labels == 1
        ), "SVMKernelSolver only works for binary_classification"
        assert train_logits is None, "SVMKernelSolver does not support train_logits"

        kernel = self.get_regularized_kernel(train_kernel)
        train_targets = (
            (train_targets + 1) / 2
        ).int()  # convert back from {-1, 1} to {0, 1}
...
class LogisticKernelSolver(BaseKernelSolver):
    ...
    def score(self, X, y, sample_weight=None):
        targets = ((y + 1) / 2).int()  # convert back from {-1, 1} to {0, 1}
        X = X.squeeze(0).cpu().numpy()
        targets = targets.cpu().numpy()
        return self.logistic_model.score(X, targets, sample_weight)
    ...
    def predict(self, eval_kernel, eval_targets, eval_logits=None, **unused_kwargs):
        ...
        log_proba = self.logistic_model.predict_log_proba(
            eval_kernel.cpu().squeeze().t().numpy()
        )
        log_proba = torch.tensor(
            log_proba, dtype=self.kernel_dtype, device=eval_kernel.device
        )
...
def solve_kernel(trainer, kernel_dict, train_logits, eval_logits):
    scores, solvers = {}, {}
    for key in ["train_ptE", "train_ftE", "train_ntk"]:
        solver = SOLVERS[trainer.args.kernel_solver](trainer.args)
        if key == "train_ntk":
            kernel = kernel_dict["train_ptE"] + kernel_dict["train_ftE"]
        else:
            kernel = kernel_dict[key]
        solver.fit(kernel, kernel_dict["train_targets"], train_logits)

        score = solver.score(kernel, kernel_dict["train_targets"])
        scores[key] = score
        solvers[key.replace("train", "solver")] = solver

    for key in ["eval_ptE", "eval_ftE", "eval_ntk"]:
        solver = solvers[key.replace("eval", "solver")]
        if key == "eval_ntk":
            kernel = kernel_dict["eval_ptE"] + kernel_dict["eval_ftE"]
        else:
            kernel = kernel_dict[key]
        score = solver.score(kernel, kernel_dict["eval_targets"])
        scores[key] = score
    return solvers, scores
