...
from transformers.file_utils import (  # is_flash_attn_2_available,; is_flash_attn_greater_or_equal_2_10,
    ModelOutput,
...
# from transformers.model.deprecated._archive_maps import (  # noqa: F401, E402
    ...
    def __init__(self, config, is_cross_attention=False, layer_idx=None):
        ...
        if self.config.lora:
            if self.is_cross_attention:
                in_feature_c = self.embed_dim
                out_feature_c = 2 * self.embed_dim
                self.c_attn = lora.LoRAConv1D(
                    in_feature_c,
                    out_feature_c,
                    config.lora_r,
                    lora_alpha=config.lora_alpha,
                )
                self.q_attn = lora.LoRAConv1D(
                    self.embed_dim,
                    self.embed_dim,
                    config.lora_r,
                    lora_alpha=config.lora_alpha,
                ...
                in_feature_c = self.embed_dim
                out_feature_c = 3 * self.embed_dim
                self.c_attn = lora.LoRAConv1D(
                    in_feature_c,
                    out_feature_c,
                    config.lora_r,
...
GPT2_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (`torch.LongTensor` of shape `(batch_size, input_ids_length)`):
            ...
            [What are input IDs?](transformers.model./glossary#input-ids)
        ...
        attention_mask (`torch.FloatTensor` of shape `(batch_size, sequence_length)`, *optional*):
            ...
            [What are attention masks?](transformers.model./glossary#attention-mask)
        token_type_ids (`torch.LongTensor` of shape `(batch_size, input_ids_length)`, *optional*):
            ...
            [What are token type IDs?](transformers.model./glossary#token-type-ids)
        position_ids (`torch.LongTensor` of shape `(batch_size, sequence_length)`, *optional*):
            Indices of positions of each input sequence tokens in the position embeddings. Selected in the range `[0,
            config.max_position_embeddings - 1]`.

            [What are position IDs?](transformers.model./glossary#position-ids)
